...
const userBookings = {
  maintenance: maintenanceBookings,
  facilities: [
    {
      id: 1,
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const payButtons = document.querySelectorAll(
    ".pay-now-button, .pay-booking-button"
  );
  ...
  payButtons.forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      if (button.classList.contains("pay-now-button")) {
        // Rent payment
        document.getElementById("payment-for").textContent =
          "Monthly Rent - Luxury Villa";
        ...
        const bookingPrice =
          bookingCard.querySelector(".booking-price").textContent;

        document.getElementById(
          "payment-for"
        ).textContent = `${bookingType} - ${bookingName}`;
        document.getElementById("payment-amount-summary").textContent =
          bookingPrice;
      }

      document.getElementById("payment-method-summary").textContent =
        document.querySelector(".method-card.active")?.textContent.trim() ||
